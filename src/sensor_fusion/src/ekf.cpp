...
namespace SensorFusion 
{
  ...
  void Ekf::correct(const Measurement &measurement) {
    ...
    // we don't want to update everyting, so we need to build matrices that only update
    ...
    for (size_t i=0; i<updateSize; ++i)
    {
      ...
      // Handle negative (read: bad) covariance in the measurement. Rather
      ...
      // the kalman gain computation will blow up. Really, no measurement
      // can be completely without error, so add a small amount in that case.
      if (measurementCovarianceSubset(i, i) < 1e-9)
      {
        FB_DEBUG("WARNING: measurement had very small error covariance for index " << updateIndices[i] <<
                  ". Adding some bias noise to maintain filter stability.\n");
        measurementCovarianceSubset(i, i) = 1e-9;
      }
    }


    // the state-to-measurement function, h, will now be a measuremtn_size x full_state_size
