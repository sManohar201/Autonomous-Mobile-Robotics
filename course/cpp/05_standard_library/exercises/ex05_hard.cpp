// Exercise 05 (Hard) - Time Window with Interpolation
// Tasks: build a deque-backed sample window, prune by time, compute mean, and
// linearly interpolate a value at a requested timestamp.
    ...
    std::optional<double> interpolate(Clock::time_point t) const {
        for (std::size_t i = 1; i < samples_.size(); ++i) {
            if (samples_[i-1].t <= t && t <= samples_[i].t) {
                double span  = std::chrono::duration<double>(samples_[i].t - samples_[i-1].t).count();
                double alpha = std::chrono::duration<double>(t - samples_[i-1].t).count() / span;
...
int main() {
    ...
    assert(w.mean() == 5.0);
    auto interp = w.interpolate(t0 + std::chrono::milliseconds{50});
    assert(interp.has_value());
    assert(interp.value() == 5.0);
