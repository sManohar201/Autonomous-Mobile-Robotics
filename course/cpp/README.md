...
| 03 | Templates & Generic Programming | Function/class templates, partial specialisation, CRTP, type traits, `if constexpr`, C++20 concepts | Generic ring buffer + type-safe physical units |
| 04 | Modern C++ | `auto`, structured bindings, lambdas, `std::function`, `optional`, `variant`, `constexpr if`, fold expressions | Config parser + compile-time state machine |
| 05 | Standard Library | Containers, iterators, algorithms, `chrono`, `string_view`, custom allocators | Sensor data pipeline with time-based windowing |
| 06 | Concurrency | Threads, mutex, `condition_variable`, `atomic`, futures, thread pool | Lock-free scan queue with producer/consumer |
| 07 | Performance & Tooling | Cache hierarchy, SIMD basics, sanitisers, GDB, `perf`, CMake advanced, benchmarking | SIMD-accelerated point cloud transform |
| 08 | Robotics Patterns | Eigen, observer pattern, plugin architecture, FSM, component entity system | Mini localisation library |
